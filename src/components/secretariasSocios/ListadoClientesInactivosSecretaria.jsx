...
const ListadoClientesInactivosSecretaria = () => {
  const {
    clientes,
    setIdClienteEditar,
    setCliente,
  ...
  useEffect(() => {
    const obtenerInfo = async () => {
      handleCargando();
      await obtenerClientesInactivosPorSede(idSedeSeleccionada);
      handleCargando();
    };
    obtenerInfo();
  }, []);
  ...
  const handleNombreClienteChange = (e) => {
    ...
    const coincidencias = clientesInactivosSede.filter(
      (cliente) =>
  ...
  const totalPages = Math.ceil(clientesInactivosSede.length / itemsPerPage);
  ...
  const currentItems = nombreFiltrado
    ? clientesFiltrado
    : clientesInactivosSede.slice(indexOfFirstItem, indexOfLastItem);
...
export default ListadoClientesInactivosSecretaria;
