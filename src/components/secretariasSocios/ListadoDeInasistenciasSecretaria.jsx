...
const ListadoDeInasistenciasSecretaria = () => {
  ...
  const totalPages = Math.ceil(inasistenciasSede.length / itemsPerPage);
  ...
  const currentItems = inasistenciasSede.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  useEffect(() => {
    const obtener = async () => {
      ...
      setFechaElegida(fechaHoy);
      await obtenerinasistenciasSede(idSedeSeleccionada, fechaHoy);
      handleCargando();
    };
    obtener();
  }, []);
  ...
  const handleAsist = async (fecha) => {
    handleCargando();
    await obtenerinasistenciasSede(idSedeSeleccionada, fecha);
    handleCargando();
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="flex">
            ...
            />
            <Button onClick={() => handleAsist(fechaElegida)}>Buscar</Button>
          </div>
        </div>
        <div className="flex">
          <div>
            ...
            <Card className="overflow-hidden xl:col-span-3">
              <CardBody className="overflow-x-scroll px-0 pb-2 pt-0">
                <table className="w-full min-w-[640px] table-auto">
                  <thead>
                    <tr>
                      {["Fecha", "Hora Clase", "Profesor", "Alumno"].map(
                        (el) => (
                          <th
                            key={el}
                            ...
                            <td className={className}>
                              <div className="flex items-center justify-center gap-4">
                                <Typography
                                  ...
                                  className="font-bold"
                                >
                                  {formatearFecha(fechaInasistencia)}
                                </Typography>
                              </div>
                            </td>
                            <td className={className}>
...
export default ListadoDeInasistenciasSecretaria;
