...
const ListadoDeClientesSecretaria = () => {
  ...
  useEffect(() => {
    const obtenerInfo = async () => {
      handleCargando();
      await obtenerClientesPorSede(idSedeSeleccionada);
      handleCargando();
    };
    obtenerInfo();
  }, []);
  ...
  const handleNombreClienteChange = (e) => {
    ...
    const coincidencias = clientesSede.filter(
      (cliente) =>
  ...
  const totalPages = Math.ceil(clientesSede.length / itemsPerPage);
  ...
  const currentItems = nombreFiltrado
    ? clientesFiltrado
    : clientesSede.slice(indexOfFirstItem, indexOfLastItem);
...
export default ListadoDeClientesSecretaria;
