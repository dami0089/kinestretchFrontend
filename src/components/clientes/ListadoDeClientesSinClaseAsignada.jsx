...
const ListadoDeClientesSinClaseAsignada = () => {
  ...
  const handleNombreClienteChange = (e) => {
    ...
    const coincidencias = clientesSinClase.filter(
      (cliente) =>
  ...
  const totalPages = Math.ceil(clientesSinClase.length / itemsPerPage);
  ...
  const currentItems = nombreFiltrado
    ? clientesFiltrado
    : clientesSinClase.slice(indexOfFirstItem, indexOfLastItem);
...
export default ListadoDeClientesSinClaseAsignada;
